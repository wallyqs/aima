...
public class TSPyMSTyAstar extends JFrame{
    ...
    public final int numeroDeNodos = 5; // 5, 20 
    public final int cuantasVeces = 15; // conecta los nodos 14, 40
    public final int radioCirculo = 5; 
    LinkedList<String> unseen;
    LinkedList<String> fringe;
    LinkedList<String> mst;
    // LinkedList<String> enumeracionMST;
    public Punto mstStructure[]; // de, para, costo
    ...
    // iba hacer la class
    // esto es para saber los vecinos de los nodos que son del mst
    public void estructurarMST(){
	mstStructure = new Punto[numeroDeNodos];

	// generar la estructura 
    	// for (int i = 0; i < mstStructure.length; i++) {

	for (int j = 0; j < mstStructure.length; j++) {
	    for (int i = 0; i < theFringe.de.size(); i++) {
		mstStructure[j].nodoquesoy = theFringe.de.get(i);
		mstStructure[j].vecinos.add(theFringe.para.get(i));
	    }
	}
	// aqui lo deje pendiente antes de enviarlo a clase ya :( Softwareeeeeeeee!!!
    }
