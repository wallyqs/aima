...
public class TSPprim4 extends JFrame{
	...
	// como los voy metiendo? creo que tengo los nodos y...
	// primero tengo que saber el tamaño del arreglo de puntos
	for (int i = 0; i < puntillos.length; i++) {
	    System.out.printf("Metiendo a UNSEEN al nodo # %d \n", i);
	    unseen.add(i+"");
	}
	// print contenido de unseen
	imprimirContenidoLista(unseen);
	
	// AQUI ES DONDE INICIALIZO EL PROCESO al meter 'a'
	...
	// buscar los vecinos de A y meterlos al fringe
	System.out.printf("fringe nodo #0:\n");
	puntillos[Integer.parseInt(fringe.get(0))].imprimirMisVecinos();

	// meter los vecinos de el que estaba en el fringe al fringe
	// y tambien quitarlos de UNSEEN 
	...
	// quitar al que estaba en el fringe del fringe el 0 y DESPUES EL MENOR
	fringe.remove("0");
	mst.add("0");
	...
	// -------------------DANGEROUS-----------
	// obtener el menor de el que esta en el fringe
	// cuidado con este, segun yo te debe de pasar el valor ejem. "0" en String, porque va a estar cambiando (si me entiendes?)
	// esto te dice cual es el menor uno de los puntos, lo necesitamos para quitarlo del fringe y ponerlo en el mst
	// Wow, esta super complicada esta linea de código, estará correcta?
	// puntillos[Integer.parseInt(mst.get(0))].vecinos.get(puntillos[Integer.parseInt(mst.get(0))].obtenerMenor() +""); 
	// debe de haber otra forma de hacer esto empezando por mst.get... creo que no yappari

	// CHECAR ESTO: OBTENER EL MENOR CAMINO QUE SE PUEDA TOMAR PARTIENDO DE UNO DE LOS NODOS DEL MST
	// CHECAR ESTO: OBTENER EL MENOR CAMINO QUE SE PUEDA TOMAR PARTIENDO DE UNO DE LOS NODOS DEL MST
	// CHECAR ESTO: OBTENER EL MENOR CAMINO QUE SE PUEDA TOMAR PARTIENDO DE UNO DE LOS NODOS DEL MST
	// CHECAR ESTO: OBTENER EL MENOR CAMINO QUE SE PUEDA TOMAR PARTIENDO DE UNO DE LOS NODOS DEL MST
	// CHECAR ESTO: OBTENER EL MENOR CAMINO QUE SE PUEDA TOMAR PARTIENDO DE UNO DE LOS NODOS DEL MST
	...
	// iria imprimiendo los valores del unseen, fringe y mst, son 3 listas
	
	// escoger el root que es el puntillo0
    }
